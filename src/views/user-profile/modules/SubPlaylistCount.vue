...
watch(
  ...
  async (newVal) => {
    ...
    if (newVal.length && !props.more) {
      ...
      //   newVal.filter((item) => item.creator.userId != uid.value)
      // );
      // more为true，由于网易云node接口并没有返回类似total总条数，所有当more为true时无法知道用户到底收藏了多少歌单和创建了多少歌单
      ...
      loading.value = false;
      console.log("用户收藏歌单(more为true时)==>", pageInfo.noMoreList);
    }
  },
...
/**
 * 循环放松请求获取用户剩余的歌单条数，没请获取limit条，知道more为false无更多数据
 * @param offset 页码偏移量
 */
  ...
  <div class="create-play-list" v-loading="loading">
    ...
    >
      <div v-for="item in list" :key="item.createTime" class="song-box">
        ...
        <div
          class="song-name oneline-hide text-xs"
